...
function getDb() {
  if (!userDb) {
    ...
    userDb = new sqlite3.Database('./user_data.db');
  }
  if (!initialized) {
    initUserDatabase();
  }
...
function initUserDatabase() {
  const db = userDb || new sqlite3.Database('./user_data.db');
  db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS spaced_repetition (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER,
      level_id INTEGER,
      question_id INTEGER,
    ...
    db.run(`CREATE TABLE IF NOT EXISTS daily_reflections (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER,
    ...
    db.run(`CREATE TABLE IF NOT EXISTS streak_recovery (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER,
